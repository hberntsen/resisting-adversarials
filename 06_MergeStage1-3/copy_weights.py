...
net3 = caffe.Net('../05_TrainStage3/deploy.prototxt', '../05_TrainStage3/stage3_iter_50000.caffemodel', caffe.TEST)
merged = caffe.Net('../00_PrototxtDeploy/generated/normal.prototxt', caffe.TEST)
    ...
    for k in source.params.keys():
        for i in range(len(source.params[k])):
            ...
            if key in target.params.keys():
                ...
                print("Warning: key %s not found in target" % key)
...
copy_params(merged, 'compare_img_100_', net3)
copy_params(merged, 'compare_img_010_', net3)
copy_params(merged, 'compare_img_001_', net3)
